...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Import the model
      const model = (await import("../model/geminiModel.js")).default;

      // Check if code was AI-generated using Gemini
      try {
        // Construct prompt for Gemini
        const prompt = `
          Analyze this ${language} code and determine the probability (as a percentage) that it was generated by an AI.
          Only respond with a number between 0 and 100. No explanation.
        ...
        const aiProbability = parseFloat(result.response.text().trim());
        
        console.log(`AI generation probability: ${aiProbability}%`);

        // If AI probability is above 90%, mark as cheating
        if (aiProbability >= 90) {
          if (existingResult) {
            existingResult.codingScore = 0;
            existingResult.isCheated = true;
          ...
          return res.status(200).json({
            isCheated: true,
            message: "This solution appears to be AI-generated.",
          });
        }
      } catch (aiError) {
        console.error("AI Detection Error:", aiError.message);
        // Continue with other checks if AI detection fails
      }
      ...
      try {
        ...
        if (plagiarismResponse.data.plagiarism_score > 80) {
          if (existingResult) {
            existingResult.codingScore = 0;
            existingResult.isCheated = true;
            await existingResult.save();
